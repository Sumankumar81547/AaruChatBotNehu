...
from Aaru import Ravan, BOT_NAME
from Aaru.database import aarubot
from Aaru.modules.helpers import (
    CLOSE_BTN,
    BACK,
    CHATBOT_BACK,
    DEV_OP,
...
async def cb_handler(_, query: CallbackQuery):
    if query.data == "HELP":
        ...
        if user_status not in [CMS.OWNER, CMS.ADMINISTRATOR]:
            ...
            is_vick = aarubot.find_one({"chat_id": query.message.chat.id})
            if not is_vick:
                await query.edit_message_text(f"❖ ᴄʜᴀᴛ-ʙᴏᴛ ᴀʟʀᴇᴀᴅʏ ᴇɴᴀʙʟᴇᴅ.")
            if is_vick:
                aarubot.delete_one({"chat_id": query.message.chat.id})
        ...
        if user_status not in [CMS.OWNER, CMS.ADMINISTRATOR]:
            ...
            is_vick = aarubot.find_one({"chat_id": query.message.chat.id})
            if not is_vick:
                aarubot.insert_one({"chat_id": query.message.chat.id})
                await query.edit_message_text(
